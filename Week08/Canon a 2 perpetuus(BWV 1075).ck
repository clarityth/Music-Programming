...
fun void play(StkInstrument instrument, int notes[], dur durs[], float volume, int upto) { // +upto
    for (0 => int i; i<upto; i++){
        playNote(instrument, notes[i], durs[i], volume);
    }
}

fun void retrograde(StkInstrument instrument, int notes[], dur durs[], float volume){ // reverse play
...
spork ~ play(piano1, MELODY, DURS, 0.5);
WN => now;
spork ~ play(piano2, MELODY, DURS, 0.5);
WN * 3 => now;
spork ~ play(piano1, MELODY, DURS, 0.5);
WN => now;
spork ~ play(piano2, MELODY, DURS, 0.5);
WN * 3 => now;
spork ~ play(piano1, MELODY, DURS, 0.5, 11);
WN => now;
spork ~ play(piano2, MELODY, DURS, 0.5, 6);
WN => now;
spork ~ playNote(piano1, C5, WN);
spork ~ playNote(piano2, E4, WN);
WN => now;